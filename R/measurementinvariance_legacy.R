...
##'
##' The original authors of this function are :
...
##'
##' The function is included in the current package for packwards compatability with an introductory tutorial on
...
##' @keywords internal
##' @export invariance_semtools
invariance_semtools <-
function (..., std.lv = FALSE, strict = FALSE, quiet = FALSE,
          fit.measures = "default", baseline.model = NULL, method = "satorra.bentler.2001"){
  lavaancfa <- function(...) {
    lavaan::cfa(...)
  }
  dotdotdot <- list(...)
  if (is.null(dotdotdot$model))
    stop("all lavaan() and lavOptions() arguments must",
  ...
  if (names(dotdotdot)[1] == "")
    names(dotdotdot)[1] <- "model"
  res <- list()
  ...
  if (class(template) == "try-error")
    stop("Configural model did not converge.")
  pttemplate <- parTable(template)
  varnames <- unique(pttemplate$rhs[pttemplate$op == "=~"])
  facnames <- unique(pttemplate$lhs[(pttemplate$op == "=~") &
                                      (pttemplate$rhs %in% varnames)])
  ngroups <- max(pttemplate$group)
  if (ngroups <= 1)
    stop("Well, the number of groups is 1. Measurement",
         " invariance across 'groups' cannot be done.")
  if (std.lv) {
    for (i in facnames) {
      pttemplate <- fixParTable(pttemplate, i, "~~", i,
                                1:ngroups, 1)
    }
    fixloadings <- which(pttemplate$op == "=~" & pttemplate$free ==
                           0)
    ...
    res$fit.configural <- try(do.call(lavaancfa, dotdotdot),
                              silent = TRUE)
  }
  else {
  ...
  if (std.lv) {
    findloadings <- which(pttemplate$op == "=~" & pttemplate$free !=
                            0 & pttemplate$group == 1)
    for (i in findloadings) {
      pttemplate <- constrainParTable(pttemplate, pttemplate$lhs[i],
                                      "=~", pttemplate$rhs[i], 1:ngroups)
    }
    for (i in facnames) {
      pttemplate <- freeParTable(pttemplate, i, "~~", i,
                                 2:ngroups)
    }
    dotdotdot$model <- pttemplate
    res$fit.loadings <- try(do.call(lavaancfa, dotdotdot),
                            silent = TRUE)
  }
  else {
    loadings <- dotdotdot
    loadings$group.equal <- c("loadings")
    res$fit.loadings <- try(do.call(lavaancfa, loadings),
                            silent = TRUE)
  }
  if (std.lv) {
    findintcepts <- which(pttemplate$op == "~1" & pttemplate$lhs %in%
                            varnames & pttemplate$free != 0 & pttemplate$group ==
                            1)
    ...
    for (i in facnames) {
      pttemplate <- freeParTable(pttemplate, i, "~1", "",
                                 2:ngroups)
    }
    dotdotdot$model <- pttemplate
    res$fit.intercepts <- try(do.call(lavaancfa, dotdotdot),
                              silent = TRUE)
  }
  else {
    intercepts <- dotdotdot
    intercepts$group.equal <- c("loadings", "intercepts")
    res$fit.intercepts <- try(do.call(lavaancfa, intercepts),
                              silent = TRUE)
  }
  if (strict) {
    if (std.lv) {
      findresiduals <- which(pttemplate$op == "~~" & pttemplate$lhs %in%
                               varnames & pttemplate$rhs == pttemplate$lhs &
      ...
      dotdotdot$model <- pttemplate
      res$fit.residuals <- try(do.call(lavaancfa, dotdotdot),
                               silent = TRUE)
      for (i in facnames) {
        pttemplate <- fixParTable(pttemplate, i, "~1",
                                  "", 1:ngroups, 0)
      }
      dotdotdot$model <- pttemplate
      res$fit.means <- try(do.call(lavaancfa, dotdotdot),
                           silent = TRUE)
    }
    else {
      residuals <- dotdotdot
      residuals$group.equal <- c("loadings", "intercepts",
                                 "residuals")
      res$fit.residuals <- try(do.call(lavaancfa, residuals),
                               silent = TRUE)
      means <- dotdotdot
      means$group.equal <- c("loadings", "intercepts",
                             "residuals", "means")
  ...
  if (!quiet)
    printInvarianceResult(res, fit.measures, baseline.model,
                          method)
  invisible(res)
}
...
printInvarianceResult <- function(FIT, fit.measures, baseline.model, method) {
  ...
  if (length(fit.measures) == 1L && fit.measures == "default") {
    ...
    if (length(lavaan::lavInspect(FIT[[1]], "test")) > 1L) {
      if (lavaan::lavInspect(FIT[[1]], "test")[[2]]$test %in% c("satorra.bentler", "yuan.bentler")) {

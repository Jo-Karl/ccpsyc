...
invariance_semtools <-
  function(..., std.lv = FALSE, strict = FALSE, quiet = FALSE,
    ...
    if (is.null(dotdotdot$model)) {
      stop(
        "all lavaan() and lavOptions() arguments must",
    ...
    if (ngroups <= 1) {
      stop(
        "Well, the number of groups is 1. Measurement",
        " invariance across 'groups' cannot be done."
      )
    }
    if (std.lv) {
      for (i in facnames) {
        pttemplate <- fixParTable(
          pttemplate, i, "~~", i,
          1:ngroups, 1
        )
      }
      ...
      for (i in fixloadings) {
        pttemplate <- freeParTable(
          pttemplate, pttemplate$lhs[i],
...
printInvarianceResult <- function(FIT, fit.measures, baseline.model, method) {
  ...
  TABLE <- do.call(lavaanLavTestLRT, c(FIT, list(
    model.names = NAMES,
    method = method
  )))
